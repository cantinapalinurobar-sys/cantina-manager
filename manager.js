...
function openDbConfig(){
  document.getElementById("cfg-url").value = localStorage.getItem("pb_sb_url")||"";
  document.getElementById("cfg-key").value = localStorage.getItem("pb_sb_key")||"";
  document.getElementById("cfg-tipologie").value = TIPOLOGIE.join("\n");
...
function _authRequired(){ return localStorage.getItem("cm_auth_required")==="1"; }
...
function _effectiveDbUser(){
  if(_authRequired() && _authState.user && localStorage.getItem("cm_auth_partition")==="1"){
...
function _authToggleRequired(on){
  localStorage.setItem("cm_auth_required", on?"1":"0");
...
async function _loadMovementsV2(){
  const { data, error } = await _sb.from("cm_movements_ledger")
    .select("id,payload,deleted").eq("user_id", _effectiveDbUser());
  if(error){
...
async function loadData(){
  ...
  try{
    ...
    if(movRows && movRows._missing){
      // Ledger non raggiungibile/assente: NON ripieghiamo più sul blob cm_movements
      // (fonte disallineata/stantia). Manteniamo la cache locale corrente — che è già
      // derivata dal ledger — in sola-lettura: nessuna scrittura movimenti finché il
      // ledger non torna (_flushMovementsV2 lancia se _movV2Available è false). Le
      // altre tabelle appena caricate dal remoto restano valide.
      ...
      // Nessun seed dal blob legacy: il ledger è la fonte unica. Se la tabella è
      // vuota (installazione nuova) => movements=[], comportamento corretto.
      const rows = movRows;
